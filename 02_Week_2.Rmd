...
birthwt %>%
  ...
  labs( x='Birthweight (g)', y='', title='Infant Birthweight') +
...
birthwt %>%
  ...
  labs( x='Birthweight (g)', title='Infant Birthweight', y='') +
  scale_y_continuous(breaks=NULL)
```
...
D <- birthwt %>% ggplot(., aes(x=bwt)) +
  ...
  labs( x='Birthweight (g)', title='Infant Birthweight')+
  geom_label(x=0, y=100, label='bins = 30')
...
ggplot(lincoln_weather, aes(x = `Max Temperature [F]`, y = `Month`)) +
  ...
  #   name = "Temp. [F]")+
  labs(title = 'Temperatures in Lincoln NE, 2016') 
```
