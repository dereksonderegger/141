...
either the `x` or `y` axis or using length. The exception to this is the heatmap, 
...
P7 <- 
  ...
  labs(x=NULL, y=NULL, title='Mosaic Plots') +
...
has a nice article showing the candidate ages. I grabbed a few of the most prominent 
candidates and pulled their birthdays from Wikipedia and then calculated their 
age on election day. As usual the data is [available](https://raw.githubusercontent.com/dereksonderegger/141/master/data-raw/Prez_Candidate_Birthdays) 
...
In this section we'll consider two categories. We have the results from a survey 
administered to students in an introductory statistics class at a New York 
university. We'll compare the number of student responses broken down by year in 
school and gender.
...
are a few more males in class. After a more detailed look, I see that the most 
...
text for the country and color for continent.
...
gapminder %>% 
  ...
  ggplot(., aes(label = country, area = pop, subgroup = long_group, subgroup2=lat_group, fill = continent)) +
  geom_treemap(layout='fixed') +
  geom_treemap_text(layout='fixed', place = "centre", grow = TRUE, colour = "white") +
  geom_treemap_subgroup_text(layout = "fixed", place = "centre") +
  geom_treemap_subgroup_border(layout = "fixed") +
