...
MathStat_Faculty_Composition %>%
  ...
  labs(title='Pie Chart: NAU Dept of Mathematics & Statistics Faculty')
```
...
MathStat_Faculty_Composition %>%
  mutate(Type = fct_collapse(Type, Lecturer  = str_c(c('','Senior ','Principle '), 'Lecturer'))) %>%
  mutate(Type = fct_collapse(Type, Professor = str_c(c('Full','Assistant','Associate'), ' Professor'))) %>%
  mutate(Type = fct_relevel(Type, c('Instructor','Lecturer','Professor'))) %>%
  group_by(Type) %>% summarize(Number = sum(Number)) %>% arrange(rev(Type)) %>%
  mutate( Text.Location = cumsum(Number) - Number/2 ) %>%
ggplot(., aes(x='', y=Number, fill=Type) ) + geom_col( size=1) +
  geom_text( aes(label=Type,   y=Text.Location), x=1.1, size=6 ) +
  geom_text( aes(label=Number, y=Text.Location+2), x=0.9, size=6 ) +
  theme_void() + theme(legend.position = 'none') + 
  coord_polar("y", start=0) + 
  labs(title='Pie Chart: NAU Dept of Mathematics & Statistics Faculty', size=10)
```
...
```{r}
  ...
  group_by(Type) %>% summarize(Number = sum(Number)) %>% arrange(rev(Type)) %>%
  mutate( Text.Location = cumsum(Number) - Number/2 ) %>%
ggplot(., aes(x='', y=Number, fill=Type) ) + geom_col( size=.5) +
  ...
  labs(title='Stacked Bar Chart: NAU Dept of Mathematics & Statistics Faculty')
...
P <- ggplot(D, aes(x=Type, y=Number, fill=Type) ) + 
  ...
  geom_text( aes(label=Number, y=Number-.7) ) +
...
ggplot(Titanic) +
  geom_mosaic(aes(x=product(Sex, Class), fill=Survived), offset=.1) +
