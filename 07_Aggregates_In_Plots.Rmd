...
**In Tableau, put all of the variables onto the shelf spaces**
**(using the average of each).**
**This gets the grid of graphs, each with one point. Assuming you have a column**
**that is a unique identifier for each observation, if you put that in the detail**
**card (which basically defines the scale at which the average is being taken),**
**then the average is taken over each observation, and we end up with the scatter**
**plot matrix.**
...
2.  We will use a smaller version of the diamonds dataset that Wilke uses in 
