...
1. Data exploration and visualization requires both a source of data and mechanism to manipulate the data and wrangle the data into a format that is usable by our graphing software. The first section (chapters 1 & 2) addresses these two issues. Then we consider examples where the choice of variable standardization matters.

2. Introduce foundational ideas about Elementary Perceptual Task order and grouping ideas. These will provide the basis to discuss why a visualization is good or bad. (chapter 3). Next we introduce a slew of graphic types and examples of where and how they are commonly used. (chapters 4-10?)

3. Important relationships between two variables might exist for one group of individuals but not another group. Exploring relationships between variables across a variety of scales and groups is crucial for understanding nuanced relationships. Interactive graphics allow for a convenient way to allow the user to move between scales.

4. Unfortunately data visualizations can be used to mislead the audience. This section outlines common malicious techniques for lying with data and how to provide transparency that such shenanigans aren't present.
